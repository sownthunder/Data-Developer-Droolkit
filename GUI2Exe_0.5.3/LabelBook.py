...
# $Revision: 83 $
# $URL: https://gui2exe.googlecode.com/svn/trunk/LabelBook.py $
# $Id: LabelBook.py 83 2012-05-07 19:42:52Z andrea.gavana $
########### GUI2Exe SVN repository information ###################
...
class ImageContainer(ImageContainerBase):
    ...
    def InitializeColors(self):
        ...
        if not ArtManager.Get().IsDark(self._colorsMap[INB_TAB_AREA_BACKGROUND_COLOR]) and \
           not ArtManager.Get().IsDark(self._colorsMap[INB_TEXT_COLOR]):
            ...
            colour = self._colorsMap[INB_TAB_AREA_BACKGROUND_COLOR]
            self._colorsMap[INB_ACTIVE_TEXT_COLOR] = wx.Colour(255-colour.Red(), 255-colour.Green(), 255-colour.Blue())
    ...
    def OnPaint(self, event):
        ...
        if self.HasFlag(INB_GRADIENT_BACKGROUND):
        ...
        underlineFont = wx.SystemSettings_GetFont(wx.SYS_DEFAULT_GUI_FONT)
        ...
        for i in xrange(len(self._pagesInfoVec)):
            ...
            if not style & INB_SHOW_ONLY_IMAGES and not self._pagesInfoVec[i].GetCaption() == "":
                ...
                if self._nIndex == i:
                    colour = self._colorsMap[INB_ACTIVE_TEXT_COLOR]
                else:
                    colour = self._colorsMap[INB_TEXT_COLOR]
...
class LabelContainer(ImageContainerBase):
    ...
    def DrawLabel(self, dc, rect, text, bmp, imgInfo, orientationLeft, imgIdx, selected, hover):
        ...
        if bmp.Ok():
            self._ImageList.Draw(self._pagesInfoVec[imgIdx].GetImageIndex(), dc,
                                 imgRect.x, imgRect.y,
                                 wx.IMAGELIST_DRAW_TRANSPARENT, True)

...
class CofA_G_Scanner(): # {
    ...
    def scan_directory(self, the_directory, ignore_dir_list, file_type_list): # {
        ...
        try: # {
            ...
            df_g_drive = pd.DataFrame(data=p1)
            ...
            df_g_drive.to_csv(outfile_path, index=False)
        # }
        except: # {
...
def main(): # {
    ...
    try: # {
        ...
        scan_directory = "F:/APPS/G Drive/C of A's/Agilent/"
        # Create PATH variable for output
        out_directory = "C:/data/outbound/CofA/"
        # create class instance
        G_Scan = CofA_G_Scanner(inbound_dir=scan_directory, outbound_dir=out_directory)
        # CALL SCAN FUNCTION/METHOD
        G_Scan.scan_directory(the_directory=G_Scan.inbound_dir,
                              ignore_dir_list=[''],
                              file_type_list=['.pdf'])
    # }
    except: # {

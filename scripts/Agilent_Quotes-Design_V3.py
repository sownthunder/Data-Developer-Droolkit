...
# 
# ### *transferred to* `"Quotes_Tracker_FINALE.py` 
# 
# in `C:/Data-Developer-Droolkit/scripts/`
# 
# **ON** `2019_11_22`
# 
# ---
...
class QuotesBook: #{
    ...
    def create_ttk_styles(self): #{
        ...
        try: #{
            ...
            # BUTTON STYLE
            self.button_style = ttk.Style().configure("TButton", padding=6, 
                                                      relief="groove", background="#96A853",
                                                      font=('Candara', 12, 'bold'))
            # ENTRY BOX STYLE
            self.entry_style = ttk.Style().configure("TEntry", padding=6, 
                                                     relief="ridge", font=('Candara', 8, 'bold'))
            # SPIN BOX STYLE
            self.spinbox_style = ttk.Style().configure("TSpinbox", padding=6,
    ...
    def create_paned_windows(self): #{
        self.test_frame = ttk.LabelFrame(master = self.root, text = "Test Frame:")
        self.test_frame.pack()
        
        self.m1 = ttk.PanedWindow(master = self.test_frame)
        self.m1.pack(fill = tk.BOTH, expand = 1)
        
        self.left = ttk.Entry(master = self.m1, bd = 2)
        self.m1.add(self.left)
        
        self.m2 = ttk.PanedWindow(master = self.m1, orient = tk.VERTICAL)
        self.m1.add(self.m2)
        
        self.top = ttk.Scale(master = self.m2, orient = tk.HORIZONTAL)
